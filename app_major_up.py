...
API_KEY      = "tnGHxeqXAnkqoV6pUL2XFjDStejcjhb2"   # Polygon/Massive (news, snapshots)
FRED_API_KEY = "fcdca4c8a04957bdcaa22c32f1e8eb34"   # FRED macro data
AV_API_KEY   = "AV7S2VM0C1BTC4GY"                   # Alpha Vantage — 20+ year OHLCV
...
_AV_SESSION  = _av_session()
_AV_CACHE: dict = {}   # ticker.upper() -> full-history DataFrame (session cache)


def fetch_ohlc(ticker: str, days: int = 0) -> pd.DataFrame:
    """
    Fetch daily adjusted OHLCV from Alpha Vantage (20+ years of history).
    Uses TIME_SERIES_DAILY_ADJUSTED with outputsize=full.
    Results are cached in _AV_CACHE so each ticker costs only 1 API call
    per server session — subsequent calls just slice the cached DataFrame.

    The `days` parameter slices the tail of the history for backwards
    compatibility with all existing callers.  days=0 returns full history.
    ...
    if sym not in _AV_CACHE:
        ...
        try:
            r = _AV_SESSION.get(AV_BASE, params=params, timeout=20)
            r.raise_for_status()
            payload = r.json()
        except Exception as exc:
            raise ValueError(f"Alpha Vantage request failed for {sym}: {exc}")
        ...
        if "Note" in payload or "Information" in payload:
            ...
            raise ValueError(
                f"Alpha Vantage rate limit reached — wait 60 seconds. ({msg[:120]})"
            )

        ts_key = "Time Series (Daily)"
        if ts_key not in payload:
            raise ValueError(
                f"Unexpected Alpha Vantage response for {sym}. "
                f"Keys: {list(payload.keys())}"
            )

        rows = []
        for date_str, bar in payload[ts_key].items():
            try:
                rows.append({
                    ...
                    "Low":    float(bar["3. low"]),
                    # Use adjusted close as Close so all indicators are
                    # split/dividend-corrected across 20+ year history
        ...
        if not rows:
            raise ValueError(f"No OHLCV data parsed for {sym}.")
        ...
        _AV_CACHE[sym] = full_df

    full = _AV_CACHE[sym]

    # Slice to requested window (days=0 → full history)
    if days and days > 0:
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fetch_ohlc() now returns Alpha Vantage split/dividend-adjusted prices
    in the 'Close' column, so all indicators automatically reflect true
    20+ year adjusted history with no survivorship distortion.
    """
    df = df.copy()
    c = df["Close"]   # already adjusted close from Alpha Vantage
    ...
    # OBV (vectorised — avoids slow Python loop over 20yr daily bars)
...
def run_backtest(ticker: str, start_date: str, end_date: str,
    ...
    # Alpha Vantage returns 20+ years — fetch full history (days=0)
    # so backtests are never artificially capped at 2 or 5 years.
    # The session cache in fetch_ohlc() means this costs only 1 API call.
    ...
    return {
        ...
        'stats': {
            ...
            'years_backtested': round((end_dt - start_dt).days / 365, 1),
            'data_source':      'Alpha Vantage (split/dividend adjusted)',
        }
    }
...
def api_technical(ticker):
    ...
    # Alpha Vantage returns 20+ years — expose the full range to the frontend.
    # days=0 means "return full history" in fetch_ohlc().
    days_map = {
        "3M":  92,
        "6M":  183,
        "1Y":  365,
    ...
    try:
        ...
        stats = {
            ...
            "prev_close":      round(close.iloc[-2], 2) if len(close) > 1 else round(close.iloc[-1], 2),
            "change":          round(close.iloc[-1] - close.iloc[-2], 2) if len(close) > 1 else 0,
            "change_pct":      round((close.iloc[-1] - close.iloc[-2]) / close.iloc[-2] * 100, 2) if len(close) > 1 else 0,
            "high_52w":        round(close.rolling(252).max().iloc[-1], 2) if len(close) >= 252 else round(close.max(), 2),
            "low_52w":         round(close.rolling(252).min().iloc[-1], 2) if len(close) >= 252 else round(close.min(), 2),
            "all_time_high":   round(close.max(), 2),
            "all_time_low":    round(close.min(), 2),
            "years_of_history":round(len(close) / 252, 1),
...
def api_correlation():
    ...
    days_map = {"6M": 180, "1Y": 365, "2Y": 730, "5Y": 1825, "10Y": 3650, "ALL": 0}
    days = days_map.get(period, 365)
    if len(tickers) < 2:
...
def api_backtest(ticker):
    try:
        # Allow "ALL" as start_date to use full Alpha Vantage history
        start_date_raw = request.args.get('start_date', '')
        if start_date_raw.upper() == 'ALL' or not start_date_raw:
            # Default: 10 years back for a meaningful long-run backtest
